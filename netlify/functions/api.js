...
app.put('/api/quotes/:id/send', authMiddleware, async (req, res) => {
  try {
    const { id } = req.params;
...
// Public routes
app.get('/api/public/quotes/:uniqueLinkId', async (req, res) => {
  try {
...
app.post('/api/public/quotes/:uniqueLinkId/accept', async (req, res) => {
  try {
    const { uniqueLinkId } = req.params;
...
app.get('/api/public/payment/success', async (req, res) => {
  try {
